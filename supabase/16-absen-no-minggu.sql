-- =============================================================
--  Absensi libur hari Minggu — hanya Senin sampai Sabtu yang boleh.
--  Menambahkan penolakan di sisi server (RPC) supaya tidak bisa
--  ditembus dengan memanggil fungsi langsung, bukan cuma di UI.
--
--  extract(dow from tanggal): 0 = Minggu, 1 = Senin, ... 6 = Sabtu.
--  Tanggal dihitung dengan zona WITA (Asia/Makassar), konsisten
--  dengan logika absen yang sudah ada.
--
--  Jalankan file ini di Supabase SQL Editor → RUN. Aman diulang.
-- =============================================================

-- ---------- check_in: tolak Minggu ----------
...
begin
  ...
  if extract(dow from today) = 0 then
    return query select false, null::int,
      'Absensi libur hari Minggu. Hanya Senin sampai Sabtu.'; return;
  end if;
