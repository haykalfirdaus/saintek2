-- =============================================================
--  ABSENSI SISWA + LOGIN SISWA + ROLE BARU (wali_kelas, guru_mapel)
--  Jalankan di SQL Editor SETELAH schema.sql, storage.sql, upload-rls.sql.
--
--  Ringkasan:
--    - Tambah role 'wali_kelas' (read-only) & 'guru_mapel' (khusus tugas).
--    - students: kolom auth_user_id (link ke akun login) + nickname.
--    - student_credentials: simpan email+password default utk dibagikan admin.
--    - attendance: catatan absen harian per siswa (dengan lat/lng + jarak).
--    - RLS: siswa hanya absen utk dirinya; admin kelola; wali_kelas baca.
--    - Koordinat sekolah + radius disimpan di app_settings (bisa diubah).
-- =============================================================

-- ---------- Perbaiki trigger auth: JANGAN buat profil utk akun siswa ----------
-- Akun siswa dibuat dgn metadata { is_student: true }. Tanpa penjagaan ini,
-- trigger lama akan memberi setiap siswa baris profiles ber-role admin.
...
-- Helper: student_id milik user yang sedang login (SECURITY DEFINER, anti-rekursi RLS).
...
-- Siswa boleh update HANYA nickname miliknya sendiri (nama & no_absen dikunci).
...
-- Izinkan siswa membaca baris siswa-nya sendiri (selain "public read" yg sudah ada).
-- (students sudah punya "students public read" using(true), jadi read aman.)
...
-- Hanya role provisioning (developer, sekretaris) yang boleh lihat kredensial.
...
--  Absen via geolocation = 'hadir' + method='geo'. Admin bisa input manual.
-- =============================================================
...
-- Baca: admin (dev/sekretaris/ketua) + wali_kelas (read-only) + siswa (miliknya).
...
-- Siswa boleh INSERT absen HANYA utk dirinya sendiri, tanggal hari ini (WITA),
-- dan hanya via method 'geo'. (Validasi radius dilakukan RPC di bawah.)
...
-- Admin (dev/sekretaris/ketua) boleh kelola penuh (input/edit/hapus manual).
...
-- =============================================================
--  RPC: absen via geolocation (validasi jarak <= radius sekolah)
--  Dipanggil dari dashboard siswa. Mengembalikan jarak (meter).
--  Pakai rumus haversine. Menolak bila di luar radius atau sudah absen.
-- =============================================================
...
-- =============================================================
--  RPC: rekap absensi rentang tanggal (dipakai panel admin)
--  Mengembalikan absensi + nama siswa dalam rentang [from, to].
-- =============================================================
...
--   - wali_kelas : READ-ONLY (sudah otomatis lewat "public read" select).
--                  TIDAK ditambahkan ke policy write mana pun → write terblokir.
--   - guru_mapel : hanya boleh WRITE di tasks. Tambah policy khusus.
-- =============================================================

-- Pengatur Kebersihan boleh mengelola piket (dan hanya piket).
...
-- Guru Mapel boleh menambah/ubah tugas (selain admin lama).
...
-- Catatan hak upload: sesuai aturan, fitur upload file HANYA untuk
-- developer/sekretaris/ketua (lihat can_upload di upload-rls.sql). Guru Mapel
-- boleh membuat tugas berbentuk TEKS, tetapi TIDAK boleh melampirkan file
-- (guard_task_attachment akan menolak). Jika nanti guru mapel perlu melampirkan
-- file, tambahkan 'guru_mapel' ke can_upload().
